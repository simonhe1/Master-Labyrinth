...
public class RotateShiftableTileTests {
	
	@Test public void oneValidRotation(){
		GameBoard gb = new GameBoard(4);
		gb.populateStaticMoveableTileArray(staticMoveableTileArray1());
		gb.setupStaticBoard();
		MoveableTile shiftable = gb.getMoveableTileArray().get(0); //shiftable Tile
		Player p = gb.getBoard()[2][2].getPlayers().get(0);
		p.rotateShiftableTile(90);
		int topExpected = 1;
		int bottomExpected = 0;
		int leftExpected = 1;
		int rightExpected = 0;
		int topActual = shiftable.getTop();
		int bottomActual = shiftable.getBottom();
		int leftActual = shiftable.getLeft();
		int rightActual = shiftable.getRight();
		assertTrue("",topExpected==topActual && bottomExpected==bottomActual 
				&& leftExpected==leftActual && rightExpected == rightActual);
	}
	
	@Test public void twoRotationsOneValidFollowedByOneInvalid(){
		GameBoard gb = new GameBoard(4);
		gb.populateStaticMoveableTileArray(staticMoveableTileArray1());
		gb.setupStaticBoard();
		MoveableTile shiftable = gb.getMoveableTileArray().get(0); //shiftable Tile
	...
	public static ArrayList<MoveableTile> staticMoveableTileArray1(){
